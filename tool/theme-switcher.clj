...
(def themes
  {:hacker
   {:alacritty {:dark "alabaster_dark"    :light "ayu_light"}
    :helix     {:dark "modus_vivendi"     :light "emacs"}
    :zellij    {:dark "retro-wave"        :light "pencil-light"}}

   :gruvbox
   {:alacritty {:dark "gruvbox_dark"      :light "gruvbox_light"}
...
(defn replace-in-file
  ...
  (spit path (str/replace-first (slurp path)
                                pattern
                                replacement)))
...
(dorun
  (map update-program-config
       (keys program-configs)))
